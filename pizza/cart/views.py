...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request, slug):
        ...
        if size_id:
            ...
            if not product_size:
                return JsonResponse({
                    'Помилка': 'Товару немає в наявності.'
                }, status=400)
        ...
        if product_size.stock < quantity:
            return JsonResponse({
                'Помилка': f'Тільки {product_size.stock} є в наявності'
            }, status=400)
        ...
        if existing_item:
            ...
            if total_quantity > product_size.stock:
                return JsonResponse({
                    'Помилка': f"Canon add {quantity} items. Only {product_size.stock - existing_item.quantity} more available."
                }, status=400)
    ...
    def post(self,request, item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({
                    'error': f'Тільки {cart_item.product_size.stock} є в наявності.'
                }, status=400)
...
class ClearCartView(CartMixin, View):
    def post(self, request):
        ...
        if request.headers.get('HX-Request'):
            return TemplateResponse(request, 'cart/cart__empty.html', {
                'cart': cart
            })
        return JsonResponse({
            'succes': True,
            'message': 'Кошик очищений'
        })
